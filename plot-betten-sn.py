...
if incidence_saxony is None:
    ...
    incidence_x = np.array(
        [
            incidence_start + datetime.timedelta(days=x)
            for x in range(len(incidence_saxony["values"]))
        ]
    )
    incidence_saxony["values"] = np.array(incidence_saxony["values"]).astype(np.double)
...
(p1,) = ax_beds.plot_date(
    ...
    "-",
    color="tab:brown",
    label="Belegt auf C19-Normalstation",
)
(p3,) = ax_beds.plot_date(
    ...
    ":",
    color=p1.get_color(),
    label="Belegt mit C19 gesamt",
)
bed_threshold = 1300
ax_beds.axhline(y=bed_threshold, color="red", linestyle="--")
ax_beds.text(
    ax_beds.get_xlim()[0] + 1, bed_threshold * 1.05, "Neue Notbremse", color="red"
)
ax_beds.text(
    0.99,
    0.02,
    "Stand: {}\nDaten von coronavirus.sachsen.de".format(
        incidence_x[-1].strftime("%x")
    ),
    ha="right",
    transform=ax_beds.transAxes,
)
